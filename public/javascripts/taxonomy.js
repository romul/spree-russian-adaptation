...
var handle_before_delete = function(li){
	...
	if (!delete_confirmed){
		jConfirm('Вы уверены, что хотите удалить этот таксон?', 'Confirm Taxon Deletion', function(r) {
			if(r){
  ...
  ui : {
    ...
    context     : [
        ...
        },
        {
            ...
            icon    : "rename.png",
            visible : function (NODE, TREE_OBJ) { if(NODE.length != 1 || NODE[0].id == 'root') return false; return TREE_OBJ.check("renameable", NODE); },
            action  : function (NODE, TREE_OBJ) { jQuery.each(NODE, function () { show_adv_edit_taxon_form(this.id); }); }
        },
        {
            id      : "delete",
            label   : "Удалить",
