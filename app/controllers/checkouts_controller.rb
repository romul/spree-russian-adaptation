class CheckoutsController < Spree::BaseController 
  ...
  update do
    ...
    success.wants.html do
      ...
      @order.complete!
      session[:order_id] = nil if @order.state == 'new'
  ...
  update.before do
    # prevent validation failure if exclude 'billing' from checkout_steps
    if params[:checkout][:shipment_attributes]
      params[:checkout][:bill_address_attributes] ||= params[:checkout][:shipment_attributes][:address_attributes]
      firstname = params[:checkout][:shipment_attributes][:address_attributes][:firstname]
      params[:checkout][:shipment_attributes][:address_attributes][:firstname] = [firstname, params[:address][:secondname]].join(' ')     
    end
  ...
  def object
    ...
    unless params[:checkout] and params[:checkout][:coupon_code]
      ...
      @object.order.shipments << Shipment.new if @object.order.shipments.empty?
